...
class WGAN_GP(BaseModel):
    ...
    def _build_train_graph(self):
        with tf.variable_scope(self.name):
            ...
            G_loss = 0.2 * tf.reduce_mean(-C_fake) + 0.8 * gen_l2_cost
            # add by xjc MSE_loss
    ...
    def _good_generator(self, z, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            ...
            net = self._residual_block(net, 8*nf, resample='same', name='res_block_s1') # 8x8x512
            net = self._residual_block(net, 4*nf, resample='up', name='res_block2') # 16x16x256
            net = self._residual_block(net, 4*nf, resample='same', name='res_block_s2') # 16x16x256
            net = self._residual_block(net, 2*nf, resample='up', name='res_block3') # 32x32x128
            net = self._residual_block(net, 2*nf, resample='same', name='res_block_s3') # 32x32x128
            net = self._residual_block(net, 1*nf, resample='up', name='res_block4') # 64x64x64
            net = self._residual_block(net, 1*nf, resample='same', name='res_block_s4') # 64x64x64
